...
int main(int argc, char const *argv[])
{
	...
	while(1){
		...
		*/
		...
		if(strcmp(msgfromserver, "exit")==0){
		...
		char caesarcifrado[MAXIMA_LONGITUD_CADENA];		
		
		//strcpy(caesarcifrado,caesarcipher(descifradorot));
		...
		int num_bytes = send(sockfd, descifradorot, strlen(descifradorot),0);
		if(num_bytes<0){
